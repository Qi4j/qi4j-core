...
public final class NamedAssociationModel
    implements NamedAssociationDescriptor, ConstraintsCheck, Serializable
{
    ...
    private NamedAssociationType namedAssociationType;
    ...
    public NamedAssociationModel( Method accessor,
    ...
    )
    {
        ...
        initialize();
        this.namedAssociationType = new NamedAssociationType( qualifiedName, getRawClass( type ).getName(), queryable );
    ...
    public NamedAssociationType namedAssociationType()
    {
        return namedAssociationType;
    }

    public <T> NamedAssociation<T> newInstance( ModuleUnitOfWork uow, EntityState state )
    {
        NamedAssociation<T> associationInstance = new NamedAssociationInstance<T>( state instanceof BuilderEntityState ? builderInfo : this, this, uow, state );

        if( TransientComposite.class.isAssignableFrom( accessor.getReturnType() ) )
        {
            associationInstance = (NamedAssociation<T>) uow.module()
    ...
    public void checkAssociationConstraints( NamedAssociation namedAssociation )
        throws ConstraintViolationException
    {
        if( associationConstraints != null )
        {
            List<ConstraintViolation> violations = associationConstraints.checkConstraints( namedAssociation );
            if( !violations.isEmpty() )
            {
                throw new ConstraintViolationException( "", "<unknown>", accessor, violations );
            }
